...
def list_find_node(node_list, node):
    for x in node_list:
        if x.junction == node.junction:
            return x
    return None
...
def run_astar(source_junction_index, target_junction_index, t0, price_func=price_function,
    ...
    while not open_list:  # while list is not empty
        ...
        for son in children_of_junction(best_node):
            ...
            if son_in_open is not None:
                if son_in_open.g_value > son_g_value:
                    ...
                    son_in_open.h_value = heuristic_func(son_in_open, target_junction, t0)
            ...
            if son_in_closed is not None:
                if son_in_closed.g_value > son_g_value:
                    ...
                    son_in_closed.h_value = heuristic_func(son_in_open, target_junction, t0)
